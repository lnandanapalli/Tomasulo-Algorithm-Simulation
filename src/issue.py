...
def issue(cycle, PC, instructions, ROB, size_ROB, 
        ...
        rat_int, rat_fp, iw): # issue width is passed to iw from main
    #====================CHANGE===============================
    for i in range(iw): # for each in range of issue_width iw
        if (PC.PC < len(instructions) and PC.valid): # check if pc is less than the total instructions so that
            #loop doesn't go beyond even if issue_width permits and check if PC is valid, PC = Program Counter
            #=================================================
