...
fu_int_adder = deque()
time_fu_int_adder = 1
# 4 cycles in fp adder 
fu_fp_adder = deque()
time_fu_fp_adder = 4
# 15 cycles in fp multiplier
fu_fp_multi = deque()
time_fu_fp_multi = 15
...
#=================== ADJUST ISSUE WIDTH ===================
# Issue width can be between from 1 - 4 inclusive
issue_width = input("Enter issue width(1-4): ")
try:
    issue_width = int(issue_width)
    if issue_width >=1 and issue_width <= 4:
        pass
    else:
        print("Wrong issue width, Using Default = 1")
        raise ValueError
except:
    issue_width = 1
